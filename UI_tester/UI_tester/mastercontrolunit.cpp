...
QList< UI_User > MasterControlUnit::getUsersList(){//start
    QList< User > users = um_->getUsers();
    QList<UI_User> ui_users;

    for(int i=0;i<users.size();i++){
        UI_User ui_user;
        ui_user.id=users.at(i).id;
        ui_user.name=users.at(i).name;
        QPixmap picture;
        picture.load(users.at(i).iconfile);
        ui_users.append(ui_user);
    }
    return ui_users;
}
