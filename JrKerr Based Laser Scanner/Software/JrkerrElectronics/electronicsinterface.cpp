...
bool ElectronicsInterface::cleanUp()
{
    ...
    if (initialized_)
    {
        ...
        QMapIterator<int,Motor*> i(motorMap_);
        while(i.hasNext()){
            i.next();
            result = result && i.value()->cleanUp();
        }
...
QList<IOControler*> ElectronicsInterface::getIOs(){
    return ioMap_.values();
}

IOControler* ElectronicsInterface::getIO(int id){
    if(ioMap_.contains(id)){
        return ioMap_[id];
    }else{
        return new IOControler();
    }
}
    ...
    for (unsigned int i = 0; i < electronicsChildren.length(); i++) {
        ...
        if ("controller"==echild.nodeName().toLower()) {
            ...
            if ("DC/Encoder"==t.text()) {
                ...
                IOControler* io = new IOControler(echild);
                ioMap_[io->id()] = io;
...
void ElectronicsInterface::createMotor(QDomNode sourceDomNode) {
        ...
        motorMap_[ID]= new Motor(ID,ADDRESS,COUNTS_PER_REV,TICKS_PER_SECOND,KP,KD,KI,IL,OL,CL,EL,SR,DB,
                                 minComPos,maxComPos, minComVel, maxComVel, minComAcc, maxComAcc);
        motorMap_[ID]->setReversed(reversed);
}
...
void ElectronicsInterface::createCoodinatedMotion() {
    ...
    for(int i=0;i<motorMap_.keys().size();i++) {
        tempM[i+1]=motorMap_[i];
    }
