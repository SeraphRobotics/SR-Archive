...
FAHLoopInXYPlane smoothLoop(FAHLoopInXYPlane loop, int times){
...
//    printf("\ntime %i",times);
    ...
    else{return smoothLoop(newloop,times);}
...
FAHLoopInXYPlane boundaryLoopFromGrid(XYGrid<float>* grid){
    ...
    for(int j=0;j<grid->ny();j++){
        ...
        if (addedRight && ! addedLeft){
            left.append(vectorFromIJ(1,j,0,grid->stepSize()));
        }else if(addedLeft && !addedRight){
            right.append((vectorFromIJ((grid->nx()-2),j,0,grid->stepSize())));
        }
