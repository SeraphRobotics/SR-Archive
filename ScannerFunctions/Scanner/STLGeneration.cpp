...
FAHLoopInXYPlane loadLoopFromXDFL(QString file)
{
    ...
    QDomElement root = xdflDom.documentElement();
    if (root.isNull()) {
        qDebug()<<"NULL ROOT";
        return FAHLoopInXYPlane();
    }
    ...
    QDomNodeList commands_ = commandsNode.childNodes();
        if (commands_.isEmpty()) {
            qDebug()<<"NO COMMANDS";
            return FAHLoopInXYPlane();
        }
    QDomElement pathTag = commands_.at(0).toElement();
    if ("path" != pathTag.nodeName().toLower()) {return FAHLoopInXYPlane();}

    QDomNodeList pathChildren = pathTag.childNodes();
